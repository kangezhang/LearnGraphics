...
11.5 化学方向扩展规划（2026-02-28，先规划不实施）
----------------------------------

目标：在既有通用框架上补齐化学课程表达能力，强调“解释型渲染”，不追求实验级仿真。
...
1.  Particle + Bond（原子/键）
2.  Geometry Constraint Field（构型约束）
3.  Electron Field（电子密度/偏向场）
4.  Energy Landscape（反应坐标与能量障碍）
5.  Reaction Timeline（反应步骤与事件）
6.  Ensemble Layer（群体概率与平衡）

分期策略：

*   Phase 1：用现有积木组合做化学课程原型
*   Phase 2：引入 `atom/bond/molecule` 语义类型与校验规则
*   Phase 3：引入化学 process（`reaction_pathway` / `equilibrium_shift` / `rate_collision`）
*   Phase 4：接入 chemistry AI 能力卡与回归快照
